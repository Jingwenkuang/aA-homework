...
class Board
  ...
  def valid_move?(start_pos)
    raise "Invalid start_pos" if start_pos < 0 || start_pos > 12
    raise "Starting cup is empty " if @cup[start_pos].empty?
  end
  ...
  def next_turn(ending_cup_idx)
    ...
    if ending_cup_idx == 6 || ending_cup_idx == 13
      ...
      :prompt
    elsif @cups[ending_cup_idx].count == 1  # only 1 stone leave in cup
